...
def save_on_master(*args, **kwargs):
    """
    Save checkpoint only on rank0, using an atomic replace to avoid other ranks
    reading partially-written files on shared filesystems.
    """
    if not is_main_process():
        return

    # torch.save(obj, f) is commonly used; keep signature compatibility
    if len(args) < 2:
        torch.save(*args, **kwargs)
        return

    obj, f = args[0], args[1]
    # If saving to a file-like object, fall back to regular torch.save
    if not isinstance(f, (str, os.PathLike)):
        torch.save(*args, **kwargs)
        return

    path = os.fspath(f)
    tmp_path = path + f".tmp.rank0.{os.getpid()}"
