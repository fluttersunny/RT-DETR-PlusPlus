...
Copyright (c) 2026 The RT-DETR++ Authors. All Rights Reserved.
---------------------------------------------------------------------------------
Modified from RT-DETRv4 (https://github.com/RT-DETRs/RT-DETRv4)
Copyright (c) 2025 RT-DETRv4 Authors. All Rights Reserved.
"""
...
@register()
class RTDETRPlusCriterion(nn.Module):
    ...
    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        if student_feature_map.shape[1] != teacher_feature_map.shape[1]:
            _logger.error(
                f"[RTDETRPlusCriterion] Feature dimension mismatch! Student: {student_feature_map.shape[1]}, Teacher: {teacher_feature_map.shape[1]}")
        ...
        if (H_s, W_s) != (H_t, W_t):
            _logger.warning(
                f"[RTDETRPlusCriterion] Resizing teacher feature map from {H_t}x{W_t} to student's {H_s}x{W_s} for distillation.")
