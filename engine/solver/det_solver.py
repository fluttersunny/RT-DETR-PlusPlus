    ...
    def fit(self, ):
        ...
        for epoch in range(start_epoch, args.epoches):
            ...
            if epoch == self.train_dataloader.collate_fn.stop_epoch:
                # Stage transition: ensure rank0 has finished writing best_stg1.pth
                # before any rank tries to read it.
                dist_utils.barrier()
                best_path = self.output_dir / 'best_stg1.pth'
                last_path = self.output_dir / 'last.pth'
                resume_path = best_path if best_path.exists() else last_path
                if dist_utils.is_main_process() and not best_path.exists():
                    print(f'[WARN] {best_path} not found, fallback to {last_path}')
